...
def _get_keybert():
    return KeyBERT(model="all-MiniLM-L6-v2") if KEYBERT_OK else None

def _tfidf(text, top_n=10):
    words = [w for w in re.findall(r'\b[a-zA-Z]{3,}\b', text.lower()) if w not in STOPWORDS]
    if not words: return []
    tf = Counter(words); total = len(words)
    scores = {w: c/total for w,c in tf.items()}
    bigrams = [f"{words[i]} {words[i+1]}" for i in range(len(words)-1)]
    for bg,c in Counter(bigrams).most_common(30): scores[bg] = (c/len(bigrams))*1.5
    ranked = sorted(scores.items(), key=lambda x: x[1], reverse=True)
    return [(k,v) for k,v in ranked if 2<=len(k.split())<=3 or len(k)>=4][:top_n]

def extract_keywords(text, top_n=10, use_keybert=True):
    kb = _get_keybert() if use_keybert else None
    if kb:
        raw = kb.extract_keywords(text, keyphrase_ngram_range=(1,2), stop_words="english", use_maxsum=True, nr_candidates=30, top_n=top_n)
        return [{"keyword": kw, "score": round(s,3), "method": "KeyBERT (semantic)"} for kw,s in raw]
    raw = _tfidf(text, top_n); mx = raw[0][1] if raw else 1
    return [{"keyword": kw, "score": round(s/mx,3), "method": "TF-IDF (fallback)"} for kw,s in raw]
